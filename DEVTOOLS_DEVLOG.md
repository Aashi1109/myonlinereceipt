...
- Devtools lives in the unified application under `app/devtools`.
- `/devtools/json-formatter` is the first working utility: it formats, minifies, validates, and presents JSON in a reader-focused input/output/inspector workspace.
...
- `/devtools` is a small Paperwork-themed tool catalog; analytics are still deliberately absent.
...
- Current workspace rule: keep Devtools code inside `app/devtools` and `lib/devtools`; create a package only after another capability genuinely needs the same code.
...
- Audited the new `app/devtools` scaffold and confirmed that no utility is shipped yet.
