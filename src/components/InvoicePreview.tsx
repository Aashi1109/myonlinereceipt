...
export default function InvoicePreview({ data }: InvoicePreviewProps) {
  ...
  return (
    <div
      className="bg-white text-slate-900 border border-slate-200 shadow-md rounded-2xl w-full p-6 md:p-8 space-y-8 select-none tracking-tight font-sans print:shadow-none print:border-0 print:p-0 print:m-0"
      id="invoice-print-area"
      style={{ minHeight: "297mm", contentVisibility: "auto" }} // standard height representation
    >
      {/* ==============================================
      ...
      {isModern && (
        <div className="space-y-6" id="template-modern-root">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
            <div className="space-y-2">
              {data.business.logo ? (
                ...
                <div className="inline-flex items-center gap-1 bg-zinc-100 border border-zinc-200 px-2 py-1 rounded text-[10px] text-zinc-600 font-bold print:hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-1 bg-zinc-55/40 border border-zinc-150 p-4 rounded-xl">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
          ...
          {(hasProjectName || hasPoNumber) && (
            <div className="grid grid-cols-2 gap-4 bg-zinc-50 border border-zinc-200/50 p-3 rounded-xl text-xs">
              {hasProjectName && (
                <div>
                  <span className="text-slate-400 font-bold block text-[9px] uppercase tracking-wider">Project</span>
                  <span className="font-bold text-slate-800">{data.invoice.projectName}</span>
                </div>
              )}
              {hasPoNumber && (
                <div>
                  <span className="text-slate-400 font-bold block text-[9px] uppercase tracking-wider">P.O. Number</span>
      ...
      {!isModern && (
        <div className="space-y-6" id="template-classic-root">
          ...
          <div className="bg-slate-50/50 p-4 rounded-lg border border-slate-100 flex flex-col sm:flex-row justify-between gap-4">
            ...
            <div className="text-xs text-slate-650 flex flex-col justify-end text-sm text-left sm:text-right space-y-0.5">
                ...
                return (
                  <tr key={item.id || idx} className="hover:bg-slate-50/40">
                    <td className="py-3 pr-4 max-w-sm align-top leading-relaxed">
                      ...
                      {item.taxable && (
                        <span className="inline-block mt-1 font-mono text-[8px] bg-slate-100 text-slate-500 px-1 py-0.2 rounded border border-slate-200">
                          Taxable
                        </span>
                      )}
                    </td>
        ...
        <div className="md:col-span-7 space-y-4">
          {data.payment.instructions && (
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-slate-600 leading-relaxed whitespace-pre-wrap max-w-md bg-zinc-50 border border-zinc-200/50 p-2.5 rounded-lg">
                {data.payment.instructions}
              </p>
            </div>
          )}
      ...
      <div className="space-y-4 pt-4 border-t border-slate-100 text-[11px] leading-relaxed text-slate-500" id="preview-terms-block">
        ...
        <div className="pt-6 text-center text-[10px] text-slate-400 border-t border-slate-100/60 flex justify-between items-center no-print">
